...
function fillCanceledItemsGroup(orderSummary) {
    ...
    if (orderLineItems.length > 0) {
        ...
        var fulfilledToAddress = {};
        if (orderSummary && orderSummary.OrderDeliveryGroupSummaries && orderSummary.OrderDeliveryGroupSummaries.records && orderSummary.OrderDeliveryGroupSummaries.records.length) {
            var orderDeliveryGroupSummary = orderSummary.OrderDeliveryGroupSummaries.records[0];
            var deliverToNameObject = splitFullName(orderDeliveryGroupSummary.DeliverToName);
            fulfilledToAddress = new Address(orderDeliveryGroupSummary.DeliverToAddress);
            fulfilledToAddress.firstName = deliverToNameObject.firstName;
            fulfilledToAddress.lastName = deliverToNameObject.lastName;
            fulfillmentOrderDetails.fulfilledToAddress = fulfilledToAddress;
        }
...
function SomOrderModel(somApiOrderSummary, somOrderToOrderItemSummariesMap, somOrderToFulfillmentMap, somOrderToOrderPaymentSummariesMap, shipmentMap) {
    ...
    if (somApiOrderSummary) {
        ...
        if (somApiFulfillmentOrders) {
            ...
            var fulfilledToAddress = {};
            if (somApiOrderSummary && somApiOrderSummary.OrderDeliveryGroupSummaries && somApiOrderSummary.OrderDeliveryGroupSummaries.records && somApiOrderSummary.OrderDeliveryGroupSummaries.records.length) {
                var orderDeliveryGroupSummary = somApiOrderSummary.OrderDeliveryGroupSummaries.records[0];
                var deliverToNameObject = splitFullName(orderDeliveryGroupSummary.DeliverToName);
                fulfilledToAddress = new Address(orderDeliveryGroupSummary.DeliverToAddress);
                fulfilledToAddress.firstName = deliverToNameObject.firstName;
                fulfilledToAddress.lastName = deliverToNameObject.lastName;
            ...
            if (orderLineItems.length > 0) {
                fulfillmentOrderDetails = {
                    ...
                    fulfillmentStatus: STATUSORDERED,
                    orderLineItems: orderLineItems,
                    fulfilledToAddress: fulfilledToAddress
                };
                item = new FulfillmentOrder(fulfillmentOrderDetails);
