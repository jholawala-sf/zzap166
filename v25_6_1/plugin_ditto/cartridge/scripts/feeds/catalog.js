...
const imageMethods = {
    transformable: { method: 'getHttpsImageURL' },
...
function imageLink(cfg, data) {
    ...
    if (cfg.hasOwnProperty('imageType')) {
        ...
        if (img) {
            return !!cfg.transformable && !!cfg.dimensions ? img[imageMethods[method].method](cfg.dimensions)
                .toString() : img[imageMethods[method].method]()
                .toString();
        }
    }
}
...
function images(cfg, data) {
    let method = !!cfg.transformable && !!cfg.dimensions ? 'transformable' : 'non-transformable';
    if (cfg.hasOwnProperty('imageType')) {
        var _images = data.Product.getImages(cfg.imageType);
        if (!empty(_images)) {
            return _images.toArray()
                .map(function (imgFile) {
                    return !!cfg.transformable && !!cfg.dimensions ? imgFile[imageMethods[method].method](cfg.dimensions)
                        .toString() : imgFile[imageMethods[method].method]()
                        .toString();
                });
        }
    }
...
function standardPrice(cfg, data) {
    ...
    } else if (!data.Product.isMaster() && data.Product.getMasterProduct() && !empty(data.Product.masterProduct.getPriceModel())) {
        priceModel = data.Product.masterProduct.getPriceModel();
    } else if (data.Product.isMaster() || data.Product.isVariationGroup()) {
        priceModel = data.Product.getVariationModel()
            .getDefaultVariant()
            .getPriceModel();
    }
...
function writeProduct(product, parameters, writeNextCB) {
    ...
    if (
        ...
        if (product.isMaster() || product.isVariationGroup()) {
            defVariant = product.getVariationModel()
                .getDefaultVariant();
        }
    ...
    if (product.isMaster() && parameters.IncludeVariationGroupProduct === true) {
        writeChildProducts(product.getVariationModel()
            .getVariationGroups(), product, writeNextCB);
    }

    // output child variants
    if (product.isMaster() && parameters.IncludeVariantProduct === true) {
        writeChildProducts(product.getVariationModel()
            .getVariants(), product, writeNextCB);
    }

    if (product.isVariationGroup() && parameters.IncludeVariantProduct === true) {
        writeChildProducts(product.getVariationModel()
            .getSelectedVariants(), product, writeNextCB);
    }
}
...
function buildProductData(product, defaultProduct) {
    var data = {
        ...
        ProductLink: require('dw/web/URLUtils')
            .abs('Product-Show', 'pid', product.ID)
            .https(),
        ImageLink: imageLink,
        Images: images,
