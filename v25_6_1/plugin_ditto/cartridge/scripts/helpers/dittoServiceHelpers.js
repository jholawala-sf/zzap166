...
const SVC_FILTER_KEYS = require('*/cartridge/scripts/util/dittoServiceFilterKeys');
...
const DittoServiceHelpers = {
    ...
    appendUrlPath: function (service, path) {
        ...
        var serviceURL = DittoServiceHelpers.getServiceUrl(service);
    ...
    parseArrayResponse: function (serviceResponse) {
        ...
        try {
            if (DittoServiceHelpers.OK_STATUS_CODES.indexOf(serviceResponse.getStatusCode()) > -1 && serviceResponse.getStatusMessage() === 'OK') {
                result.success = true;
            }
        } catch (ex) {
            Logger.error(ex.toString() + ' in ' + ex.fileName + ':' + ex.lineNumber);
    ...
    parseTextResponse: function (serviceResponse) {
        ...
        try {
            if (DittoServiceHelpers.OK_STATUS_CODES.indexOf(serviceResponse.statusCode) > -1 && serviceResponse.statusMessage === 'OK') {
                result.success = true;
            }
    ...
    isIterable: function (element) {
        ...
        return DittoServiceHelpers.isArray(element) || DittoServiceHelpers.isObject(element);
    },

    iterate: function (object, parent) {
        if (DittoServiceHelpers.isIterable(object)) {
            DittoServiceHelpers.forEachIn(object, function (key, value) {
                if (key !== null) {
                ...
                DittoServiceHelpers.iterate(value, parent);
            });
        }
        return object;
    },
...
module.exports = DittoServiceHelpers;
