        ...
        if (window.sfpp) {
            sfpp.ready(function () {   // eslint-disable-line
                ...
                $('body').on('checkout:confirmPayment', function (e, data) {
                    const remainingPaymentAmount = 'remainingPaymentAmount' in data ? data.remainingPaymentAmount : null;
                    if (remainingPaymentAmount == null || remainingPaymentAmount > 0) {
                        payments.updateBillingDetails(data.billingDetails);
                        payments.confirm().then(function () {
                            // Clear out error message
                            errorElement.empty();
                        ...
                        // SF Payments not needed, execute callback
                        data.callback({ remainingPaymentAmount: remainingPaymentAmount });
                    }
                });
            });
        }
