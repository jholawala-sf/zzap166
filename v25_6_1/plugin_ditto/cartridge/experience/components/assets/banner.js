...
module.exports.render = function (context) {
    ...
    if (content.mobImage) {
        const { focalPointX2, focalPointY2 } = ImageTransformation.getScaledImage(content.mobImage);

        model.mobImage = {
            src: {
                mobile: ImageTransformation.url(content.mobImage.file, { device: 'mobile' }),
                desktop: ImageTransformation.url(content.mobImage.file, { device: 'desktop' })
            },
            alt: content.mobImage.file.getAlt(),
            focalPointX: focalPointX2,
            focalPointY: focalPointY2
        };
    } else {
        model.mobImage = (model.image) ? model.image: null;
    }
