...
/**
 * Sends passwordless login notification
 * @param {SynchronousPromise} promise
 * @param {CustomerNotification} data
 * @returns {SynchronousPromise}
 */
function passwordlessLogin(promise, data) {
    if (data.params.containsKey('url')) { // SFRA compat
        data.PasswordlessLoginLink = data.params.url;
    } else {
        data.PasswordlessLoginLink = URLUtils.https('PasswordlessLogin-Login', 'token', data.params.PasswordlessToken);
    }
    return sendTrigger(hookPath + 'passwordlessLogin', promise, data);
}
...
function triggerDefinitions() {
    ...
    return {
        ...
        passwordlessLogin: {
            description: 'Passwordless Login trigger',
            attributes: mergeCustomer([
                'PasswordlessToken',
