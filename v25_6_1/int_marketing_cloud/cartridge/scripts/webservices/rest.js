...
RestService.prototype = {
    ...
    /**
     * Section: Data Events
     * Name: Upsert data rows into a data extension
     * Documentation: https://developer.salesforce.com/docs/atlas.en-us.noversion.mc-apis.meta/mc-apis/postDataExtensionRowsetByKey.htm
     * Endpoint: /hub/v1/dataevents/key:{key}/rowset
     * Method: POST
     * Content Type: JSON
     * Request Headers: Authorization
     * Request Args: dataExtensionId, customerKey, rows
     */
    dataEventsRowset: function messagingDeliveryRecords() {
        var restSvc = this;
        return LocalServiceRegistry.createService('marketingcloud.rest.dataevents.rowset', {
            /**
             * Create request for upserting records into a data extension
             * @param {dw.svc.HTTPService} svc
             * @param {string} sendID
             * @param {string} customerKey
             * @param {string} recipientSendID
             */
            createRequest: function(svc, dataExtensionId, customerKey, rows) {
                restSvc.setAuthHeader(svc);

                var svcURL = restSvc.token.restInstanceURL,
                    svcPath = '/hub/v1/dataevents/key:{key}/rowset';

                if (!empty(dataExtensionId)) {
                    svcPath = svcPath.replace('{key}', dataExtensionId);
                } else if (!empty(customerKey)) {
                    svcPath = svcPath.replace('{key}', 'key:' + customerKey);
                }
                ...
                var params = JSON.stringify(rows.toArray());
