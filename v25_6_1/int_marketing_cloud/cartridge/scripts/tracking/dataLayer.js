...
(function mcDataLayerInit(){
    ...
    if (analyticsEnabled) {
        if (empty(dataLayer.setOrgId)) {
            var parentMid = curSite.getCustomPreferenceValue('mcMIDParent');
            var mid = curSite.getCustomPreferenceValue('mcMID');

            if (parentMid) {
                dataLayer.setOrgId = [parentMid, mid];
            } else {
                dataLayer.setOrgId = mid;
            }
        }
    ...
    switch(eventName) {
        ...
        case 'product':
            if (empty(dataLayer.trackPageView)) {
                ...
                if (!empty(product)) {
                    dataLayer.trackPageView = {
                      item: product.isMaster() || product.isOptionProduct() ? product.ID : product.masterProduct.ID
                    };
