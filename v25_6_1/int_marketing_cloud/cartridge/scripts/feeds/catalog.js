...
/**
 * Returns xml string for product variant attributes
 * @param cfg
 * @param data
 * @returns {String|void}
 */
function attributes(cfg, data) {
    if (data.Product.isVariant()) {
        const variationModel = data.Product.variationModel;
        const allAttributes = variationModel.productVariationAttributes;

        let attributesXML = '<attributes>';
        helpers.forEach(allAttributes, function (attr) {
            var selectedValue = variationModel.getSelectedValue(attr);
            attributesXML += '<attribute>';
            attributesXML += '<name>' + attr.displayName + '</name>';
            attributesXML += '<value>' + selectedValue.displayValue + '</value>';
            attributesXML += '</attribute>';
        });
