  ...
  if (isIterable(object)) {
    forEachIn(object, function (key, value) {
      if (key !== null) {
        ...
        } else if (FILTER_KEYS.indexOf(String(key).toLowerCase()) > -1) {
 ...
 * Assumes that the ServiceType in business manager is set to Http
 *
...
function generateServiceCallback() {
  return {
    ...
    createRequest: function (service, params) {
      ...
      var serviceOptions = params

      service.setRequestMethod(serviceOptions.method)
 ...
 * useAbsoluteURL - if set to true, replaces the entire service URL with the url passed in; if false, appends the url to service.URL; defaults to false
 *
...
function fetch(serviceName, url, options) {
  ...
  if (serviceConfiguration.getServiceType() !== 'HTTP') {
    throw new TypeError(
      'Service ' +
        serviceName +
        ' is not of type HTTP. The service type must be HTTP in Business Manager.'
    )
  }
  ...
  if (isNaN(serviceParameters.timeout)) {
    ...
    if (serviceProfile && serviceProfile.getTimeoutMillis()) {
      ...
      serviceParameters.timeout = config.FETCH_DEFAULT_TIMEOUT
    }
  }
  ...
  if (serviceParameters.useCredentials && credentials) {
    serviceParameters.headers.authorization =
      ...
      StringUtils.encodeBase64(
        [credentials.user, credentials.password].filter(Boolean).join(':')
      )
  }
  ...
  serviceParameters.url = options.useAbsoluteURL
    ? serviceParameters.url
    : service.URL + serviceParameters.url
  ...
  if (
    (typeof response.body === 'string' || response.body instanceof String) &&
    utils.isJSONString(response.body)
  ) {
