 ...
 */
base.createCookie = base.createCookie || function createCookie(name, value, age) {
 ...
 */
base.getCookie = base.getCookie || function getCookie(name) {
 ...
 */
base.removeCookie = base.removeCookie || function removeCookie(name, resp) {
    var cookies = request.getHttpCookies();
    var cookieCount = cookies.getCookieCount();
    for (var i = 0; i < cookieCount; i++) {
        if (name === cookies[i].getName()) {
 ...
 */
base.setCookiesToResponse = base.setCookiesToResponse || function setCookiesToResponse(cookiesToSet, response) {
    Object.keys(cookiesToSet).forEach(function (key) {
        ...
        if (!valueParts || valueParts.length < 2) {
            response.addHttpCookie(base.createCookie(key, value, maxAge));
            return;
        }
        ...
        valueParts.forEach(function (valuePart) {
            ...
            var cookie = base.createCookie(cookieKey, valuePart, maxAge);
 ...
 */
base.getSLASAccessTokenForGuestSessionBridge = base.getSLASAccessTokenForGuestSessionBridge || function (sessionId) {
    ...
    if (!slasServiceUtils.isPrivateClient()) {
        ...
        sessionBridgeAccessTokenParameters.code_verifier = authorizeResponse.code_verifier;
    }
    ...
    return slasAuthService.getSessionBridgeAccessToken(sessionBridgeAccessTokenParameters);
};
 ...
 * @returns {Object} object containing SLAS tokens.
 */
base.getSLASAccessTokenForRefreshLogin = base.getSLASAccessTokenForRefreshLogin || function getSLASAccessTokenForRefreshLogin(refreshToken) {
 ...
 */
base.getOCAPISessionBridgeCookies = base.getOCAPISessionBridgeCookies || function getOCAPISessionBridgeCookies(accessToken, ipAddress) {
 ...
 */
base.logoutCustomer = base.logoutCustomer || function logoutCustomer(refreshToken, accessToken) {
 ...
 */
base.isValidRequestURI = base.isValidRequestURI || function isValidRequestURI(requestURI) {
    var isValidURI = true;
