 ...
 */
base.encodeToString = base.encodeToString || function encodeToString(object) {
    if (!object) {
        return '';
    }
    var encodedString = '';
 ...
 */
base.deserializeQueryString = base.deserializeQueryString || function deserializeQueryString(queryString) {
 ...
 */
base.throwHttpError = base.throwHttpError || function throwHttpError(response, description) {
    var error = {
        type: 'HTTP',
...
function generateServiceCallback() {
    return {
        execute: function (service, params) {
            ...
            service.client = client; // eslint-disable-line no-param-reassign
 ...
 */
base.fetch = base.fetch || function fetch(serviceName, url, options) {
    ...
    if (serviceParameters.body) {
        if (
            ...
            serviceParameters.body = base.encodeToString(options.body);
        } else {
            serviceParameters.body = JSON.stringify(options.body);
        }
    }

    var queryString = base.encodeToString(serviceParameters.queryParameters);
    if (queryString) {
